# encoding: utf-8
# author: xinhchen (xinhchen-2@my.cityu.edu.hk)
# author: zxding (d.z.x@qq.com)
...
tf.app.flags.DEFINE_integer('max_pair_num', 10, 'max number of tokens per documents')
## model struct ##
...
def build_model(word_embedding, x, sen_len, doc_len, keep_prob1, keep_prob2, y, l, el, filter_sizes, RNN=biLSTM):
    ...
    loss_class = tf.constant(0, tf.float32)
    reg_label = tf.constant(0, tf.float32)
    reg_elabel = tf.constant(0, tf.float32)
    ...
    with tf.variable_scope('sequence_prediction'):
        # Use CNN to encode neighbor information
        h_flat, o_feature = cnn_part(s_c, filter_sizes)

        if FLAGS.softmax_or_crf:
            # Use softmax to get the final labels
            pred_class, reg_class = softmax_part(h_flat, FLAGS.max_doc_len, o_feature, keep_prob2, FLAGS.n_class, 'softmax_w_class', 'softmax_b_class')
            valid_num = tf.cast(tf.reduce_sum(doc_len), dtype=tf.float32)
            loss_class = -tf.reduce_sum(y * tf.log(pred_class)) / valid_num
        else:
            # Use CRF to get the final labels
            # First through a fully-connected layer
            s1 = tf.reshape(h_flat, [-1, o_feature])
    ...
    reg = reg_label + reg_elabel + reg_class
    loss = loss_label + loss_class + loss_elabel

    return loss, pred_class, reg


def cnn_part(s_c, filter_sizes):
    # Input: clause level embedding matrix
    # Output: Embedding matrix after encoding neighbor information
    cnn_features = []
    for i, filter_size in enumerate(filter_sizes):
        s_class = tf.pad(s_c, [[0, 0], [i+1, i+1], [0, 0]])
        s_class = tf.expand_dims(s_class, -1)
        filter_shape = [filter_size, 4 * FLAGS.n_hidden, 1, FLAGS.num_filters]
        ...
        cnn_features.append(h)

    # Combine all the pooled features
    num_filters_total = FLAGS.num_filters * len(filter_sizes)
    h_pool = tf.concat(cnn_features, 3)
    # h_pool's size
    h_flat = tf.reshape(h_pool, [-1, FLAGS.max_doc_len, num_filters_total])
    return h_flat, num_filters_total
