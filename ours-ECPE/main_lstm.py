...
def build_model(word_embedding, x, sen_len, doc_len, keep_prob1, keep_prob2, y, l, el, filter_sizes, RNN=biLSTM):
    ...
    with tf.name_scope('elabel_prediction'):
    ...
    with tf.name_scope('sequence_prediction'):
        # Use another BiLSTM to encode the context information
        h_flat = RNN(s_c, doc_len, n_hidden=FLAGS.n_hidden, scope=FLAGS.scope + 'class_sentence_layer')
        o_feature = 2 * FLAGS.n_hidden
...
def main(_):
    ...
    FLAGS.scope = 'lstm_softmax'
    run()

    FLAGS.softmax_or_crf = 0
    FLAGS.scope = 'lstm_crf'
    run()
