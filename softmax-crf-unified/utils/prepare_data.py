# encoding:utf-8
# author: xinhchen (xinhchen2-c@my.city.edu.hk)
...
import pdb, time, logging, datetime, sys
...
def load_data(input_file, word_idx, label2id, emo2id, max_doc_len = 75, max_sen_len = 45):
    ...
    while True:
        ...
        for pair in pairs:
            if pair[0] == pair[1]:
                ...
                if y_tmp[pair[0]-1][label2id["B-"+emolabeldict[pair[0]-1]]] != 1:
    ...
    y, x, sen_len, doc_len = map(np.array, [y, x, sen_len, doc_len])
    for var in ['y', 'x', 'sen_len', 'doc_len']:
        print('{}.shape {}'.format( var, eval(var).shape ))
    print('n_cut {}'.format(n_cut))
    print('load data done!\n')
    return doc_id, y_pairs, x, sen_len, doc_len, y
